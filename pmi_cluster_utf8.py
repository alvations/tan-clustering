...
'''
A utf8 version of @mheilman's pmi_cluster.py
...
def deaccent(text):
  """ Remove accentuation from string. """
  return unicodedata.normalize('NFKD', text).encode('ascii','ignore').encode('utf8')

def document_generator(path, lower=False):
  """ Yields tokens from documents with tokens seperated by whitespaces. """
  with codecs.open(path,'rb','utf8') as fin:
    yield deaccent(fin.read().replace(u'\xae',' (R) ')).split()
